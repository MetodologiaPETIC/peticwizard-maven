...
public class SubareasRESTService {
	...
	/**
	 * Implementa a listagem de subáreas, que é retornada ao cliente quando o mesmo
	 * requisita o endereço <code>/subareas</code> usando o método HTTP {@link GET}
	 * .
	 * 
	 * @return uma mensagem do Protocol Buffers do tipo
	 *         {@link SubareasProtos.Subareas}, da qual pode ser obtida uma lista de
	 *         {@link SubareasProtos.Subarea}, cada uma representando uma subárea
	 *         cadastrada na base de dados.
	 */

	@GET
	public SubareasProtos.Subareas listarTodasAsSubareas() {
		List<Subarea> subareasJPA = daoSubarea.listarTodos();
		SubareasProtos.Subareas.Builder subareasPB = SubareasProtos.Subareas.newBuilder();
		for (Subarea subareaJPA : subareasJPA) {
			subareasPB.addSubarea(SubareasProtos.Subarea.newBuilder()
					.setId(subareaJPA.getId())
					.setNome(subareaJPA.getDescricao())
					.setAreaId(subareaJPA.getArea().getId().toString())
					.build());
		}
	...
	public SubareasProtos.Subarea consultarSubarea(@PathParam("id") String id) {
		...
		if (subareaJPA == null) {
			...
			return SubareasProtos.Subarea.newBuilder()
					.setId(subareaJPA.getId())
					.setNome(subareaJPA.getDescricao())
					.setAreaId(subareaJPA.getArea().getId().toString()).build();
		}
	}
	...
	public ProcessosProtos.Processos listarTodosOsProcessos(
		...
		for (ProcessoGenerico processoJPA : subareaJPA.getProcessosGenericos()) {
			ProcessosProtos.Processo.Builder processoPB = ProcessosProtos.Processo
					.newBuilder().setId(processoJPA.getId())
					.setNome(processoJPA.getNome())
					.setSubareaId(processoJPA.getSubarea().getId());
			if (processoJPA.getDescricao() != null)
				processoPB.setDescricao(processoJPA.getDescricao());
			processosPB.addProcesso(processoPB.build());
		}
